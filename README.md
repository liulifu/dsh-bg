...
- 设置入口：设置页 → 通用（General）→「外观」下方 →「背景」行
...
- **动态壁纸（MP4）**：上传 MP4 视频作为背景，静音循环播放；视频 Blob 存 IndexedDB
  ...
  - 渲染：插件注入 `<video id="dsh-bg-video">` 到当前模式对应的表面（frame / 侧栏 / 详情栏），
    `z-index:-1` 位于内容之下，`object-fit:cover` 铺满；模糊/透明度直接作用于视频元素
...
- 「模糊度」滑杆：0-24px，给背景图片/视频加高斯模糊（毛玻璃效果，不影响文字清晰度）
  - 实现：图片绘制在表面的 `::before` 伪元素上（blur/opacity 只作用于图片），
    深色遮罩在 `::after`，文字与设置菜单完全不受影响
- 「文字颜色」手动调整（配合背景图片提升可读性）：
  - 灰度模式（默认）：一个滑杆，黑(0) → 白(255)，整体调文字明度
  - RGB 模式（开关，默认关）：开启后显示 R/G/B 三个滑杆精确调色
  - 通过覆盖主题文字令牌（`--dsw-alias-label-primary/secondary/tertiary/caption`）实现，
    次级文字自动带透明度保持层次；只作用于有背景的面板范围
- 「移除背景」一键清除当前背景（保留图片库）
